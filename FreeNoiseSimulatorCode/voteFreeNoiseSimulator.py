...
import pandas as pd
from GSI import calculate_GSI_MPS_parallel, create_feature_map, remove_low_M_gates, pegasusQSVCTrain, pegasusQSVCTest
import numpy as np
...
qc.decompose().draw('mpl', filename='./circuitsQSVM/circuit_base_Vote_FreeNoiseSimulator.pdf')

# Use a sample from X_train to calculate metrics
time_init = time.time()
metrics = calculate_GSI_MPS_parallel(qc.decompose(), X_train.iloc[0].values)
time_end = time.time()
...
results_df.to_csv('resultsVote.csv')
...
best_accuracy_circuit.draw('mpl', filename='./circuitsQSVM/circuit_best_accuracy_Vote_FreeNoiseSimulator.pdf')
...
if not candidates_time.empty:
    ...
    best_time_circuit.draw('mpl', filename='./circuitsQSVM/circuit_best_time_Vote_FreeNoiseSimulator.pdf')
else:
...
best_ranking_circuit.draw('mpl', filename='./circuitsQSVM/circuit_best_ranking_Vote_FreeNoiseSimulator.pdf')
